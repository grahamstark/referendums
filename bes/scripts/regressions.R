...
for( dtype in seq( 1 , 2, by = 1 )){
       ...
       if( dtype == 1 ){
                sink( "outputs/regressions.core_samples.log" );
                print( " CORE SUBSAMPLES ");
                ...
                sink( "outputs/regressions.full_sample.log" );
                print( "FULL SUBSAMPLES ");
                # Scotland only core subsets, waves 3 (indie) and 9 (brexit)
                #
                besScotW3 <- bes[ which( bes$scotland ), ] # missing values will kill the rest
                besScotW9 <- bes[ which( bes$scotland ), ]
                # GB wave 9 core
                besFullW9 <- bes;
                besScotFull <- bes[ which( bes$scotland ), ]
